...
function EditProfilePage({ onReturn }) {
  const { getCurrentToken, deleteSavedItems, changeRoute } =
  ...
  useEffect(() => {
    if (
      UserData.current.user.related.facebook !== null &&
      UserData.current.user.related.facebook !== ""
    ) {
      setInputEditUserFacebook(UserData.current.user.related.facebook);
    }
    if (
      UserData.current.user.related.instagram !== null &&
      UserData.current.user.related.instagram !== ""
    ) {
      setInputEditUserInstagram(UserData.current.user.related.instagram);
    }
    if (
      UserData.current.user.related.tiktok !== null &&
      UserData.current.user.related.tiktok !== ""
    ) {
      setInputEditUserTiktok(UserData.current.user.related.tiktok);
    }
    if (
      UserData.current.user.related.x !== null &&
      UserData.current.user.related.x !== ""
    ) {
      setInputEditUserX(UserData.current.user.related.x);
    }
    if (
      UserData.current.user.related.youtube !== null &&
      UserData.current.user.related.youtube !== ""
    ) {
      setInputEditUserYoutube(UserData.current.user.related.youtube);
    }
  }, [UserData]);
  ...
  const handleSave = async () => {
    if (inputValidation()) {
      const inputEditUser_F =
        inputEditUserFacebook === "" ? null : inputEditUserFacebook;
      const inputEditUser_I =
        inputEditUserInstagram === "" ? null : inputEditUserInstagram;
      const inputEditUser_T =
        inputEditUserTiktok === "" ? null : inputEditUserTiktok;
      const inputEditUser_X = inputEditUserX === "" ? null : inputEditUserX;
      const inputEditUser_Y =
        inputEditUserYoutube === "" ? null : inputEditUserYoutube;
        ...
        >
          ...
          <div className="GeneralInputContainer">
            ...
            {errorInputTiktok === false ? (
              <span style={{ color: "#0063FF" }} className="errorText">
